...
- Eureka instance-id: `서비스명:실제포트` (예: `notifications-service:51234`) — 기동 로그와 Eureka 대시보드에서 확인
...
| reports-service | `reports_db` |

| notifications-service | `notifications_db` |



로컬 JDBC URL 예시 (호스트·포트·스키마는 `.env.example` 참고):



```

jdbc:mysql://<host>:<port>/notifications_db?useSSL=false&allowPublicKeyRetrieval=true&serverTimezone=Asia/Seoul&characterEncoding=UTF-8

```



DB 사용자명·비밀번호는 `.env` 파일에 설정하세요 (예: `SPRING_DATASOURCE_PASSWORD=your-password-here`).
...
# 2. .env 설정 후 Notifications Service 실행

cd ../notifications-service && ./gradlew bootRun
...
- IDE에서 실행할 때는 **Run Configuration** `NotificationsServiceApplication` 을 사용하거나, `NotificationsServiceApplication.java` 에서 main 실행 시 Working directory 가 프로젝트 루트인지 확인하세요.

- `application-local.yml` 이 `optional:file:.env[.properties]` 를 import 하므로 IDE main 실행 시에도 `.env` 가 로드됩니다.



Eureka Dashboard (`http://localhost:8761`) 에서 `NOTIFICATIONS-SERVICE` 등록을 확인합니다.
...
| `SPRING_DATASOURCE_URL` | MySQL JDBC URL (`notifications_db` 스키마) |
...
com.sparta.notifications_service/
